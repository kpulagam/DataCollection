...
public class ClassData {
	...
	/*				if (setdata.getClassMethodMap().isEmpty()) {
					listOfMethods = new HashMap<String, String>();
					setdata.setClassName(tmpClassName);
					listOfMethods.put(currentTestRun.getTestMethod()
							.getMethodName(), Integer.toString(testResult
							.getStatus()));
					...
					setdata.setClassMethodMap(setdata.getClassName(),
						...
						&& !setdata.getClassMethodMap().containsKey(
								tmpClassName)) {
					listOfMethods.put(currentTestRun.getTestMethod()
							.getMethodName(), Integer.toString(testResult
							.getStatus()));
					...
					setdata.setClassMethodMap(tmpClassName, listOfMethods);
						...
						&& setdata.getClassMethodMap()
								.containsKey(tmpClassName)) {
					listOfMethods = new HashMap<String, String>();
					listOfMethods = setdata.getClassMethodMap().get(
							tmpClassName);
					listOfMethods.put(currentTestRun.getTestMethod()
							.getMethodName(), Integer.toString(testResult
							.getStatus()));
		...
		for(String className : setdata.getAllClassMethodMap().keySet()){
			...
			for(Entry<String, Integer> method1: setdata.getAllClassMethodMap().get(className).entrySet()){
